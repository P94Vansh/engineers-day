...
export default function Dashboard() {
  ...
  const loadData = () => {
    ...
    fetchNewData().then((newdata) => {
      localStorage.setItem("newdata", JSON.stringify(newdata.data));
    });
    // return ()=>{};
  };
  ...
  const handleVerify = (transaction_id,email,event,transactionAmount, newValue, setDisabled) => {
    console.log({ transaction_id,email,event,transactionAmount, newValue });
    setDisabled(true);
    ...
    fetch("/api/update-data", {
      ...
      method: "PATCH",
      body: JSON.stringify({ transactionId:transaction_id, isVerified: newValue,email,event,transactionAmount: transactionAmount}),
    })
      .then(async (res) => {
        console.log(res)
        if (res.ok) {
          ...
          switch (response.success) {
            case true:
              data[index].isVerified = newValue;
              setData([...data]);
            ...
            {data.map((row, rowIndex) => (
              ...
              >
                {headers.map((header, colIndex) => (
                  ...
                  >
                    {header.toLowerCase() === "isverified" ? (
                      <VerifyButton
                        value={row[header]}
                        onClick={(newValue, setDisabled) =>
                          handleVerify(row["transaction_id"],row["email"],row["event"],row["transition_amount"], newValue, setDisabled)
                        }
                      />
