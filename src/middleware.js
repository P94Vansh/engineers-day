...
export function middleware(req) {
  ...
  const protectedPaths = ['/api/get-data', '/dashboard','/api/update-data'];
  if (protectedPaths.some(path => pathname.startsWith(path)) && ['GET', 'DELETE','PATCH'].includes(method)) {
...
export const config = {
  matcher: ['/api/get-data', '/dashboard','/api/update-data'],
};